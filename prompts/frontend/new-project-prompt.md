...
You are setting up Claude Code configuration for a brand new frontend project. The project uses the shared parent CLAUDE.md (typically at `<workspace>/.claude/CLAUDE.md`, one level above the project root) and the `craft-skills` plugin.
...
Read the parent workspace `CLAUDE.md` (at `../.claude/CLAUDE.md` relative to the project root) to understand what is already covered. Your project CLAUDE.md must NOT duplicate any of this content.
