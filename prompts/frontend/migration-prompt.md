...
You are migrating this project to use the `craft-skills` plugin and the shared parent CLAUDE.md (typically at `<workspace>/.claude/CLAUDE.md`, one level above the project root).
...
Read the parent workspace `CLAUDE.md` (at `../.claude/CLAUDE.md` relative to the project root) to understand what is already covered at the shared level. Your project CLAUDE.md must NOT duplicate any content from the parent. Only include project-specific information.
...
3. Verify parent CLAUDE.md is accessible: `cat ../.claude/CLAUDE.md | head -3` (adjust path if the project is not directly under the shared workspace)
