...
# Check 3: model is installed (extract model short name for grep)
MODEL_SHORT=$(echo "$LLM_MODEL" | sed 's|.*/||')
if ! "$LMS" ls 2>/dev/null | grep -q "$MODEL_SHORT"; then
  echo "LLM_UNAVAILABLE: model $LLM_MODEL not installed"
  exit 0
fi

# Auto-load model with correct context length (warm up ~10s)
LOADED=$("$LMS" ps 2>/dev/null | grep -c "$MODEL_SHORT")
if [ "$LOADED" -eq 0 ]; then
  "$LMS" load "$LLM_MODEL" -c "$LLM_CONTEXT_LENGTH" 2>/dev/null
else
  # Reload if context too small
  CTX=$("$LMS" ps 2>/dev/null | grep "$MODEL_SHORT" | grep -oE '\b[0-9]{4,6}\b' | head -1)
...
If profile is `claude+ace`, after the Gemma review completes, triage findings and skip directly to Step 1.11 (user review). Do NOT dispatch the opus agent or the parallel LLM review — Gemma handles both roles.
...
When `CRAFT_PROFILE` is `claude+ace`, SKIP the sonnet agent plan review below. Instead, run Gemma as the sole reviewer:
...
If profile is `claude+ace`, after the Gemma review completes, triage findings and skip directly to Step 2.5 (save and approve). Do NOT dispatch the sonnet agent or the parallel LLM review.
...
Expected: All 5 scripts listed (llm-agent.sh, llm-analyze.sh, llm-check.sh, llm-implement.sh, llm-review.sh)
